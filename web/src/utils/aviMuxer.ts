...
export async function createAviFromPngFrames(
    ...
    // ── RIFF header ──
    writeFourCC(view, pos, 'RIFF'); pos += 4;
    view.setUint32(pos, riffContentSize, true); pos += 4;
    writeFourCC(view, pos, 'AVI '); pos += 4;

    // ── hdrl LIST ──
    writeFourCC(view, pos, 'LIST'); pos += 4;
    view.setUint32(pos, hdrlContentSize, true); pos += 4;
    writeFourCC(view, pos, 'hdrl'); pos += 4;

    // ── avih chunk (AVIMAINHEADER, 56 bytes) ──
    writeFourCC(view, pos, 'avih'); pos += 4;
    view.setUint32(pos, avihDataSize, true); pos += 4;

    const microSecPerFrame = Math.round(1_000_000 / fps);
    view.setUint32(pos, microSecPerFrame, true); pos += 4;  // dwMicroSecPerFrame
    view.setUint32(pos, 0, true); pos += 4;                 // dwMaxBytesPerSec
    view.setUint32(pos, 0, true); pos += 4;                 // dwPaddingGranularity
    view.setUint32(pos, 0x10, true); pos += 4;              // dwFlags (AVIF_HASINDEX)
    view.setUint32(pos, frameCount, true); pos += 4;        // dwTotalFrames
    view.setUint32(pos, 0, true); pos += 4;                 // dwInitialFrames
    view.setUint32(pos, 1, true); pos += 4;                 // dwStreams
    view.setUint32(pos, maxFrameSize, true); pos += 4;      // dwSuggestedBufferSize
    view.setUint32(pos, width, true); pos += 4;             // dwWidth
    view.setUint32(pos, height, true); pos += 4;            // dwHeight
    // dwReserved[4]
    ...
    // ── strl LIST ──
    writeFourCC(view, pos, 'LIST'); pos += 4;
    view.setUint32(pos, strlContentSize, true); pos += 4;
    writeFourCC(view, pos, 'strl'); pos += 4;

    // ── strh chunk (AVISTREAMHEADER, 56 bytes) ──
    writeFourCC(view, pos, 'strh'); pos += 4;
    view.setUint32(pos, strhDataSize, true); pos += 4;

    writeFourCC(view, pos, 'vids'); pos += 4;               // fccType
    writeFourCC(view, pos, 'MPNG'); pos += 4;               // fccHandler
    view.setUint32(pos, 0, true); pos += 4;                 // dwFlags
    view.setUint16(pos, 0, true); pos += 2;                 // wPriority
    view.setUint16(pos, 0, true); pos += 2;                 // wLanguage
    view.setUint32(pos, 0, true); pos += 4;                 // dwInitialFrames
    view.setUint32(pos, 1, true); pos += 4;                 // dwScale
    view.setUint32(pos, fps, true); pos += 4;               // dwRate
    view.setUint32(pos, 0, true); pos += 4;                 // dwStart
    view.setUint32(pos, frameCount, true); pos += 4;        // dwLength
    view.setUint32(pos, maxFrameSize, true); pos += 4;      // dwSuggestedBufferSize
    view.setUint32(pos, 0xFFFFFFFF, true); pos += 4;        // dwQuality (-1)
    view.setUint32(pos, 0, true); pos += 4;                 // dwSampleSize
    // rcFrame: left, top, right, bottom (each WORD = 2 bytes)
    view.setInt16(pos, 0, true); pos += 2;                  // left
    view.setInt16(pos, 0, true); pos += 2;                  // top
    view.setInt16(pos, width, true); pos += 2;              // right
    view.setInt16(pos, height, true); pos += 2;             // bottom

    // ── strf chunk (BITMAPINFOHEADER, 40 bytes) ──
    writeFourCC(view, pos, 'strf'); pos += 4;
    view.setUint32(pos, strfDataSize, true); pos += 4;

    view.setUint32(pos, 40, true); pos += 4;                // biSize
    view.setInt32(pos, width, true); pos += 4;              // biWidth
    view.setInt32(pos, height, true); pos += 4;             // biHeight
    view.setUint16(pos, 1, true); pos += 2;                 // biPlanes
    view.setUint16(pos, 32, true); pos += 2;                // biBitCount (RGBA)
    writeFourCC(view, pos, 'MPNG'); pos += 4;               // biCompression
    view.setUint32(pos, 0, true); pos += 4;                 // biSizeImage
    view.setInt32(pos, 0, true); pos += 4;                  // biXPelsPerMeter
    view.setInt32(pos, 0, true); pos += 4;                  // biYPelsPerMeter
    view.setUint32(pos, 0, true); pos += 4;                 // biClrUsed
    view.setUint32(pos, 0, true); pos += 4;                 // biClrImportant

    // ── movi LIST ──
    writeFourCC(view, pos, 'LIST'); pos += 4;
    view.setUint32(pos, moviContentSize, true); pos += 4;
    writeFourCC(view, pos, 'movi'); pos += 4;

    // ── Video frame chunks ('00dc') ──
    for (const frame of frames) {
        writeFourCC(view, pos, '00dc'); pos += 4;
        view.setUint32(pos, frame.byteLength, true); pos += 4;
...
export async function extractPngFramesFromAvi(file: File): Promise<Blob[]> {
    ...
    const riffId = readFourCC(view, pos); pos += 4;
    if (riffId !== 'RIFF') {
    ...
    const aviId = readFourCC(view, pos); pos += 4;
    if (aviId !== 'AVI ') {
    ...
    while (pos < arrayBuffer.byteLength - 8) {
        const chunkId = readFourCC(view, pos); pos += 4;
        const chunkSize = view.getUint32(pos, true); pos += 4;
        ...
        if (chunkId === 'LIST') {
            ...
            if (listType === 'movi') {
                ...
                while (pos < moviEnd - 8) {
                    const frameId = readFourCC(view, pos); pos += 4;
                    const frameSize = view.getUint32(pos, true); pos += 4;
