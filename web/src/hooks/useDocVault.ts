...
export function useDocVault() {
    ...
    const encodeFile = useCallback(async (file: File, password: string): Promise<{ blob: Blob; isVideo: boolean }> => {
        try {
            ...
            const { blob, isVideo } = await encodeOneFile(file, password, (progress, message) => {
            ...
            setStatus({ type: 'success', message: `Encoded successfully! (${formatSize(blob.size)} ${frameLabel})` });
            return { blob, isVideo };
        } catch (err: any) {
     ...
     */
    const encodeBatch = useCallback(async (
        items: BatchFileItem[],
        password: string,
        ...
        try {
            setStatus({ type: 'processing', message: 'Loading encryption engine...', progress: 5 });
            await loadWasm();
        } catch (err: any) {
            setStatus({ type: 'error', message: 'Failed to load encryption engine' });
            return;
        }
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            onFileUpdate(item.id, { status: 'processing', progress: 10, message: 'Starting...' });
            ...
            try {
                ...
                const errMsg = err?.message || typeof err === 'string' ? String(err) : 'Unknown error';
                onFileUpdate(item.id, {
                    status: 'error',
                    progress: 0,
                    message: errMsg,
        ...
        if (batchCancelledRef.current) {
            ...
            setStatus({ type: successCount > 0 ? 'success' : 'error', message: summary });
        }
    }, [loadWasm]);
